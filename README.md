...
2. During the first execution the interface will take a while to receive the data, since there is no data in the datamart. When you know that the app has received a the first hotel events (through the terminal), click on the buttons that appear in each tab called "refresh tab".
